...
- Estudante: publicar projectos, comentar, votar, abrir dúvidas, pedir mentoria, gerir perfil/KYC, conversar quando houver conexao ou mentoria ativa.
...
- CSRF: validação aplicada em acções sensiveis de chat, conexoes, KYC, perfil, dúvidas, comentarios, votos, likes, investimento e candidatura a mentor.
...
- Dashboard do investidor: evitado erro JavaScript quando pagamentos estao desativados e o modal de investimento não e renderizado.
- Investimento: `interface_programacao/projects/invest_project.php` agora aceita JSON e FormData, exige perfil `investor`, KYC verificado, projecto publico/aprovado, impede investimento no próprio projecto e respeita minimo/maximo.
...
- Notificacoes: `mark_notification_read.php` agora também aceita JSON com `project_id`, como o dashboard do investidor envia.
- Endpoint temporario: `debug_upload.php` ficou restrito a admin.
- Uploads: dúvidas com imagem, comprovativos de investimento e CV de candidatura a mentor passaram a usar `Security::storeUploadedFile`.
- Erros internos: endpoints sensiveis deixam de devolver mensagens SQL/excecoes ao cliente; detalhes ficam no `error_log`.

## Lacunas restantes

- CSRF: ainda falta cobrir algumas paginas admin standalone que não carregam o header global; usar `getCSRFHiddenInput()` nos formularios antigos.

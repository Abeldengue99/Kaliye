<?php
// administracao/users/project_specialist_applications.php
session_start();
$admin_base = '../';
$base_url = '../../';
...
// Fetch all project specialist applications
try {
    $query = "
        SELECT psa.*, p.title as project_title, u.full_name as specialist_name, u.profile_pic, u.specialization_tags
...
<head>
    ...
    <title>Candidaturas de Especialistas a Projectos - KALIYE Admin</title>
    ...
    <style>
        .psa-card { background: rgba(15,23,42,0.6); border: 1px solid rgba(255,255,255,0.05); border-radius: 20px; padding: 1.5rem; margin-bottom: 1.5rem; }
        .psa-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 1rem; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 1rem; }
    </style>
    <?php 
...
<body>
    <div class="container-admin">
        ...
        <main class="admin-content">
            <div style="padding: 2rem;">
                <h1>Candidaturas de Especialistas a Projectos</h1>
                <p style="color: var(--text-secondary); margin-bottom: 2rem;">Gerencie as candidaturas de especialistas para participar em projetos.</p>
                        ...
                        <?php foreach ($apps as $app): ?>
                            <div class="psa-card">
                                <div class="psa-header">
                                    <div>
                                        <h3><?php echo htmlspecialchars($app['specialist_name']); ?></h3>
                                ...
                                <div style="background: rgba(255,255,255,0.02); padding: 1rem; border-radius: 10px; margin-bottom: 1rem; font-size: 0.9rem;">
                                    ...
                                    <?php if ($app['relevant_experience']): ?>
                                        <p><strong>Experiência Relevante:</strong> <?php echo nl2br(htmlspecialchars($app['relevant_experience'])); ?></p>
                                    <?php endif; ?>
                                    <?php if ($app['proposed_support']): ?>
                                        <p><strong>Suporte Proposto:</strong> <?php echo nl2br(htmlspecialchars($app['proposed_support'])); ?></p>
                                    <?php endif; ?>
                                    <?php if ($app['specialization_areas']): ?>
                                        <p><strong>Áreas de Especialização:</strong> <?php echo nl2br(htmlspecialchars($app['specialization_areas'])); ?></p>
                                    <?php endif; ?>
                                </div>
                ...
                <?php else: ?>
                    <div style="text-align: center; padding: 3rem; background: rgba(15,23,42,0.3); border-radius: 20px;">
                        <p style="color: var(--text-secondary);">Sem candidaturas de especialistas no momento.</p>
                    </div>
                <?php endif; ?>
            </div>
        </main>
    ...
    function updateApplicationStatus(applicationId, newStatus) {
        fetch('../../interface_programacao/admin/admin_process_specialist_application.php', {
            method: 'POST',

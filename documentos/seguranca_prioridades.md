# Segurança KALIYE: estado e prioridades

## Ja existe no codigo
- Prepared statements via PDO na maior parte dos endpoints.
- Hash de senhas com `password_hash` e validação com `password_verify`.
...
- KYC manual com documentos, selfie e fila de aprovação.
- Headers basicos de segurança: CSP, HSTS em HTTPS, X-Frame-Options, nosniff.

## Implementado agora
- Regeneracao de sessão apos login bem-sucedido.
- `last_auth_at` para exigir autenticacao recente em acções sensiveis.
- Fingerprint basico do dispositivo na sessão.
...
- KYC com autenticacao recente e validação forte de ficheiros.
...
## Prioridade 2: segurança antifraude e trust
- Trust Score por usuario usando KYC, denuncias, avaliacoes, histórico e comportamento.
...
## Itens que ainda não fazem sentido implementar direto no codigo PHP atual
- Honeypots inteligentes: melhor na borda/WAF ou em servicos isolados.
- Microservicos: e uma decisao de arquitetura, não uma correcao rapida.

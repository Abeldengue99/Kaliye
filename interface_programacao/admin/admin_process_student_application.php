<?php
/**
 * interface_programacao/admin/admin_process_student_application.php
 * API para processar candidaturas de estudantes
 */
...
try {
    ...
    $app_stmt = $db->prepare("SELECT * FROM project_student_applications WHERE application_id = ?");
    ...
    // Get student and project info for notification
    $student_stmt = $db->prepare("SELECT full_name FROM users WHERE user_id = ?");
    $student_stmt->execute([$application['student_id']]);
